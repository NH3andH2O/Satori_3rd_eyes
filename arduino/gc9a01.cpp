...
uint8_t GC9A01::GC9A01_update()
{
	...
	if (this->isUpdate_eyesR)
	{
		...
		if (dt > 0.001)
		{
			...
			/* 二階系統控制 */
			double acceleration = -2 * this->zeta_eyesR * this->omega_n_eyesR * this->deyesR -
								  this->omega_n_eyesR * this->omega_n_eyesR * (this->eyesR - this->target_eyesR);
	...
	if (this->isUpdate_lightMax)
	{
		...
		if (dt > 0.001)
		{
			...
			/* 二階系統控制 */
			double acceleration = -2 * this->zeta_lightMax * this->omega_n_lightMax * this->dlightMax -
								  this->omega_n_lightMax * this->omega_n_lightMax * (this->lightMax - this->target_lightMax);
