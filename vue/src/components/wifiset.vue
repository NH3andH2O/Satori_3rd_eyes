...
<template>
	...
	<div class="content">
		<p>{{ $t('softAP_name') }}</p>
		<el-input v-model="softAP.config.value.ssid" placeholder="SSID" style="width: auto" clearable
	...
	<transition name="slide-toggle">
		<div v-show="softAP.config.value.is_change_password">
			<div class="content">
				<p>{{ $t('password') }}</p>
				<el-input v-model="softAP.config.value.password" type="password" :placeholder="t('password_info')"
					style="width: auto" clearable :disabled="softAP.config.value.isLoading" />
			</div>
			<div class="content">
				<p>{{ $t('password_confirm') }}：</p>
				<el-input v-model="softAP.config.value.password_confirm" type="password"
					:placeholder="t('password_confirm')" style="width: auto" clearable
					:disabled="softAP.config.value.isLoading" @blur="passwordConfirmBlur = true"
	...
	<div class="content button">
		<el-button round @click="softAP.update()" :loading="softAP.config.value.isSaving"
			:disabled="softAP.config.value.isLoading || (softAP.config.value.password !== softAP.config.value.password_confirm && softAP.config.value.is_change_password)">{{
				$t('save')
			}}</el-button>
	</div>
</template>
